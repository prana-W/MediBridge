...
import { Calendar, Clock, User, Pill, FileText, CheckCircle, AlertCircle, Heart, ChevronDown, ChevronUp, Loader2, Stethoscope } from 'lucide-react';
...
export default function DoctorAppointments() {
  ...
  const fetchAppointments = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/auth/doctor/appointments`, {
        method: 'GET',
        credentials: 'include',
        headers: {
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20">
            <Loader2 className="h-12 w-12 animate-spin mb-4" style={{ color: '#4A90E2' }} />
        ...
        <section className="mb-12">
          <div className="flex items-center mb-6">
            <AlertCircle className="h-8 w-8 mr-3" style={{ color: '#4AD2CC' }} />
              ...
              >
                    ...
                    <div className="flex-1">
                      ...
                      {appointment.patient?.age && (
                        <p className="text-xs mt-1" style={{ color: '#333333', opacity: 0.6 }}>
                          Age: {appointment.patient.age} years
                        </p>
                      )}
                    </div>
                    <div className="px-3 py-1 rounded-full text-xs font-semibold" 
                         style={{ backgroundColor: '#4AD2CC20', color: '#4AD2CC' }}>
                      Pending
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex items-center">
                      <Calendar className="h-5 w-5 mr-3" style={{ color: '#4A90E2' }} />
                    ...
                    {appointment.patient?.phone && (
                      <div className="flex items-center">
                        <User className="h-5 w-5 mr-3" style={{ color: '#4AD2CC' }} />
          ...
          {upcomingAppointments.length === 0 && (
            <div className="text-center py-12">
              <Calendar className="h-16 w-16 mx-auto mb-4" style={{ color: '#4AD2CC', opacity: 0.3 }} />
              <p className="text-lg" style={{ color: '#333333', opacity: 0.6 }}>
        ...
        <section>
          <div className="flex items-center mb-6">
            <CheckCircle className="h-8 w-8 mr-3" style={{ color: '#4A90E2' }} />
            <h2 className="text-3xl font-bold" style={{ color: '#333333' }}>
              Past Appointments
            </h2>
          </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    {appointment.checkedAt && (
                      <div className="flex items-center">
                        <CheckCircle className="h-5 w-5 mr-3" style={{ color: '#4AD2CC' }} />
                        <span className="text-sm" style={{ color: '#333333' }}>
                          Checked at: {formatTime(appointment.checkedAt)}
                        </span>
                      </div>
                    )}
                    ...
                    <button
                      onClick={() => toggleCard(appointment._id)}
                    ...
                    {/* Expanded Content */}
                    {expandedCard === appointment._id && (
                      <div className="mt-4 pt-4 border-t space-y-4" style={{ borderColor: '#4A90E240' }}>
                        
                        {/* Next Checkup */}
                        {appointment.nextCheckup && (
                          <div className="p-3 rounded-lg" style={{ backgroundColor: '#4AD2CC20' }}>
                            <div className="flex items-center mb-2">
                              <Calendar className="h-5 w-5 mr-2" style={{ color: '#4AD2CC' }} />
                              <span className="font-semibold" style={{ color: '#4AD2CC' }}>
                                Next Checkup Scheduled
                              </span>
                            </div>
                            <p className="text-sm font-medium ml-7" style={{ color: '#333333' }}>
                              {formatDate(appointment.nextCheckup)}
                            </p>
                          </div>
                        )}

                        {/* Bed Allotment Status */}
                        {appointment.isBedAlloted && (
                          <div className="p-3 rounded-lg" style={{ backgroundColor: '#4A90E220' }}>
                            <div className="flex items-center">
                              <CheckCircle className="h-5 w-5 mr-2" style={{ color: '#4A90E2' }} />
                        ...
                        {appointment.medications && appointment.medications.length > 0 && (
                          <div>
                            <div className="flex items-center mb-3">
                              <Pill className="h-5 w-5 mr-2" style={{ color: '#4A90E2' }} />
                              <span className="font-semibold" style={{ color: '#4A90E2' }}>
                            ...
                            <div className="space-y-2 ml-7">
                              {appointment.medications.map((med, index) => (
                                <div 
                                  ...
                                  style={{ backgroundColor: 'white', borderColor: '#4A90E240' }}
                                >
                                  <p className="font-semibold mb-1" style={{ color: '#333333' }}>
                                    {med.medication?.name || 'Medication'}
                                  </p>
                                  {med.medication?.dosage && (
                                    <p className="text-xs mb-2" style={{ color: '#4AD2CC' }}>
                                      {med.medication.dosage}
                                    </p>
                                  )}
                                  <div className="flex gap-4 text-xs" style={{ color: '#333333', opacity: 0.7 }}>
                                    <span>📅 {med.totalDays} days</span>
                                    <span>⏰ {med.timesPerDay}x per day</span>
                                  </div>
                                </div>
                              ))}
                            </div>
                        ...
                        {/* Remarks */}
                        {appointment.remarks && (
                          <div className="p-3 rounded-lg" style={{ backgroundColor: '#F2F2F2' }}>
                            <div className="flex items-start">
                              <FileText className="h-5 w-5 mr-2 mt-0.5" style={{ color: '#4A90E2' }} />
                              <div className="flex-1">
                                <span className="font-semibold block mb-1" style={{ color: '#4A90E2' }}>
                                  Your Remarks
                                </span>
          ...
          {pastAppointments.length === 0 && (
            <div className="text-center py-12">
              <CheckCircle className="h-16 w-16 mx-auto mb-4" style={{ color: '#4A90E2', opacity: 0.3 }} />
              <p className="text-lg" style={{ color: '#333333', opacity: 0.6 }}>
