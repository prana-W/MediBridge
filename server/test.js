...
dotenv.config(); // Make sure your .env has MONGODB_URI
...
const seedHospitals = async () => {
    try {
        ...
        await Hospital.deleteMany(); // Optional: clears old data
        console.log('🧹 Cleared old hospital data.');

        await Hospital.insertMany(
            hospitals.map(h => ({
                name: h.name.toLowerCase().trim(),
                state: h.state.toLowerCase().trim()
            }))
        );
